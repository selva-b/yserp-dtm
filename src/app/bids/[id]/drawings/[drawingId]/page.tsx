...
export default function DrawingDetailsPage() {
  ...
  const { data: bid } = useBid(bidId) // Fetch bid data for bid_number
  ...
  const hasActiveUploads = hasActiveUploadsForDrawing('bid', bidId, drawingId)
  ...
  // console.log('=== INITIAL FILES DISPLAY (v0.0 Open) - BIDS ===')
  ...
  useEffect(() => {
    const activeSessions = getActiveSessionsForPage('bid', bidId, drawingId);
  ...
  const handleRemoveFile = async (fileUrl: string, fileName: string) => {
    try {
      ...
      // Call API to delete file from Azure (hierarchical uploads)
      // This endpoint deletes files directly from Azure Storage without database lookup
      // Used for files uploaded via resumable upload in hierarchical folder structure
      // IMPORTANT: Properly encode the fileUrl parameter to avoid double-encoding issues
      const response = await apiClient(
        getApiUrl(`v1/drawings/revision-history/files/delete-from-azure?fileUrl=${encodeURIComponent(fileUrl)}`),
        {
          method: 'DELETE',
        }
      );
      ...
      // Update parent state to remove the file from UI
      // This ensures the file disappears from the display section below the dropzone
      setUploadedFiles(prev => ({
        cadd: prev.cadd.filter(file => file.url !== fileUrl),
        pdf: prev.pdf.filter(file => file.url !== fileUrl),
      }));

      console.log(`[REMOVE FILE] Updated uploadedFiles state to remove file from UI`);
    } catch (error) {
    ...
    <AppLayout>
              ...
              <ol className="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li>
                  <div className="flex items-center">
                    ...
                    <Link href={`/bids/${bidId}`} className="ml-1 text-gray-700 hover:text-blue-600 md:ml-2">
                      Bid Details
                    </Link>
                  </div>
                </li>
                    ...
                    <button
                      onClick={() => {
                        if (isEditing) {
                          ...
                          router.push(fromTasks && taskId ? `/tasks/${taskId}` : `/bids/${bidId}`)
                        }
                      }}
                      className="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50"
                    >
                      {isEditing ? 'Back to Drawing' : (fromTasks ? 'Back to Task' : 'Back to Bid')}
                    </button>
                    {!isEditing && (
                      <>
                        <button
                      ...
                      <>
                        ...
                        <div className="mt-6 pt-6 border-t border-gray-200">
                          ...
                          {(() => {
                            ...
                            // Show both sections if available
                            return (
                              <div className="space-y-4">
                                ...
                                {hasRevisionFiles && (
                                  <div className="space-y-3">
                                    <div className="bg-gray-50 border border-gray-200 rounded-md p-4">
                                      <h4 className="text-sm font-medium text-gray-700 mb-3">Revision Files</h4>
                    ...
                    {statusData.status === 'Internal review' && (
                      ...
                      <InlineFileUpload
                        ...
                        onExistingFilesDetected={(files) => {
                          ...
                          // Note: We don't update uploadedFiles here to avoid conflicts with file deletion
                          // The InlineFileUpload component already displays existing files
                          if (files.length > 0) {
        ...
        onClose={(hasUploadedFiles) => {
          ...
          if (!hasUploadedFiles) {
            if (user?.role === 'Draftsman' && fromTasks && taskId) {
              ...
              router.push(`/bids/${bidId}?tab=drawings`)
            }
          }
        }}
      ...
      {isUploadMinimized && minimizedUploadData && (
        <UploadProgressIndicator
          ...
          onClick={() => {
            // Reopen the modal with the restored session
            setShowResumableModal(true)
            setIsUploadMinimized(false)
          }}
        />
      ...
      {showMultiFileUploadModal && multiFileUploadContext && (
        <MultiFileUploadModal
          ...
          bidCode={bid?.bidNumber} // Pass bid number for folder structure
