...
export default function ProjectReportViewPage() {
  ...
  // Only fetch tasks after we have loaded tickets for this project
  ...
  // Filter tasks by project tickets and selected tickets/drawings (client-side filter)
  const filteredTasks = useMemo(() => {
    ...
    console.log('All ticket IDs for project:', allTicketIds)
    console.log('Selected ticket IDs:', selectedTicketIds)
    console.log('Selected drawing IDs:', selectedDrawingIds)

    // First, filter to only tasks belonging to this project's tickets
    // If project has no tickets, return empty array (not all tasks!)
    if (allTicketIds.length === 0) {
      console.log('No tickets for this project, showing no tasks')
      return []
    }

    tasks = tasks.filter((task) => task.ticketId && allTicketIds.includes(task.ticketId))
    console.log('After project ticket filter:', tasks.length)
  ...
  // Calculate metrics - only for tasks belonging to this project
  const metrics = useMemo(() => {
    const allTasks = tasksData?.data || []
    // Filter to only tasks belonging to this project's tickets
    const projectTasks = allTasks.filter((task) => task.ticketId && allTicketIds.includes(task.ticketId))
  ...
  const drawingBody = (rowData: any) => {
    ...
    return (
      <Link
        href={`/projects/${projectId}/drawings/${rowData.drawing.id}`}
        className="text-blue-600 hover:text-blue-800 hover:underline text-sm"
      >
        ...
        <div className="min-h-screen bg-gray-50 py-8">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="bg-red-50 border border-red-200 rounded-lg p-4">
              <p className="text-red-800">Error loading project: {errorProject.message}</p>
            </div>
          </div>
        </div>
    )
        ...
        <div className="bg-white border-b border-gray-200">
          <div className="px-6 py-3">
            <nav className="flex items-center text-sm text-gray-500">
              <Link href="/reports/projects" className="hover:text-gray-700 hover:underline">
                Projects
              </Link>
              <span className="mx-2">/</span>
              <span className="text-gray-900">{project?.projectNumber || '...'}</span>
              <span className="mx-2">/</span>
              <span className="text-gray-900 font-medium">Project Report</span>
            </nav>
          </div>
        </div>
            ...
            <div className="p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-3 mb-6">
                ...
                <div>
                  ...
                  <dd className="mt-1 text-sm text-gray-900">
                    {project?.draftingManager?.fullName || '—'}
                  </dd>
                </div>
                ...
                <div>
                  ...
                  <dd className="mt-1 text-sm text-gray-900">
                    {project?.projectClosingDate
                      ? new Date(project.projectClosingDate).toLocaleDateString()
                      : '—'}
                  </dd>
                </div>
                ...
                <div>
                  ...
                  <dd className="mt-1 text-sm text-gray-900">{project?.industry || '—'}</dd>
                </div>
                <div className="md:col-span-2">
                  <dt className="text-sm font-medium text-gray-500">Location</dt>
                  <dd className="mt-1 text-sm text-gray-900">
                    {[project?.cityTown, project?.state, project?.country].filter(Boolean).join(', ') ||
                      '—'}
                  </dd>
                </div>
              </div>
