...
import AddDrawingDrawer from '@/components/projects/AddDrawingDrawer'
import BulkUploadDrawingsModal from '@/components/projects/BulkUploadDrawingsModal'
import FilesTab from '@/components/projects/FilesTab'
import TicketsTab from '@/components/projects/TicketsTab'
...
export default function ProjectDetailsPage() {
  ...
  return (
      ...
      <div className="px-6">
          ...
          <ol className="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li>
              <div className="flex items-center">
                ...
                <span className="ml-1 text-gray-500 md:ml-2">{project?.projectNumber || 'Loading...'}</span>
              </div>
            </li>
          </ol>
        </nav>
        ...
        {isLoading && (
          <div className="bg-blue-50 border border-blue-200 rounded-md p-4 mb-6">
            <div className="flex items-center">
              ...
              <p className="text-sm text-blue-800">Loading project details...</p>
            </div>
          </div>
        )}
        ...
        <div className="bg-white shadow rounded-lg p-6 mb-6">
          <div className="flex justify-between items-start">
            ...
            {hasPermission('projects.edit:update') && (
        ...
        <div className="bg-white shadow rounded-lg mb-6">
          ...
          <div className="p-6">
                ...
                <div>
                  ...
                  <dl className="space-y-3">
                    <div>
                      ...
                      <dd className="mt-1 text-sm text-gray-900">{project.projectNumber}</dd>
                    </div>
                    <div>
                      <dt className="text-sm font-medium text-gray-500">Project Name</dt>
                      <dd className="mt-1 text-sm text-gray-900">{project.projectName}</dd>
                    </div>
                    <div>
                      <dt className="text-sm font-medium text-gray-500">Industry</dt>
                      <dd className="mt-1 text-sm text-gray-900">{project.industry || 'N/A'}</dd>
                    </div>
                    <div>
                      <dt className="text-sm font-medium text-gray-500">Project Closing Date</dt>
                      <dd className="mt-1 text-sm text-gray-900">
                        {project.projectClosingDate ? new Date(project.projectClosingDate).toLocaleDateString() : 'N/A'}
                      </dd>
                    </div>
                  </dl>
                </div>
                ...
                <div>
                  ...
                  <dl className="space-y-3">
                    <div>
                      ...
                      <dd className="mt-1 text-sm text-gray-900">{project.endUser?.companyName || 'N/A'}</dd>
                    </div>
                    <div>
                      <dt className="text-sm font-medium text-gray-500">Main Contractor</dt>
                      <dd className="mt-1 text-sm text-gray-900">{project.mainContractor?.companyName || 'N/A'}</dd>
                    </div>
                    <div>
                      <dt className="text-sm font-medium text-gray-500">Consultant</dt>
                      <dd className="mt-1 text-sm text-gray-900">{project.consultant?.companyName || 'N/A'}</dd>
                    </div>
                    <div>
                      <dt className="text-sm font-medium text-gray-500">Location</dt>
                      <dd className="mt-1 text-sm text-gray-900">
                        {[project.cityTown, project.state, project.country].filter(Boolean).join(', ') || 'N/A'}
                      </dd>
                    </div>
                  </dl>
                </div>
                      ...
                      <dd className="mt-1 text-sm text-gray-900">
                        {project.clientType ? (
                          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                            {project.clientType.replace('_', ' ').replace(/\b\w/g, (l: string) => l.toUpperCase())}
                          </span>
                        ) : (
                          'N/A'
                        )}
                ...
                <div>
                  ...
                  <dl className="space-y-3">
                    <div>
                      ...
                      <dd className="mt-1 text-sm text-gray-900">{project.projectManager?.fullName || 'N/A'}</dd>
                    </div>
                    <div>
                      <dt className="text-sm font-medium text-gray-500">Drafting Manager</dt>
                      <dd className="mt-1 text-sm text-gray-900">{project.draftingManager?.fullName || 'N/A'}</dd>
                    </div>
                  </dl>
                </div>
                ...
                <div>
                  ...
                  <dl className="space-y-3">
                    <div>
                      ...
                      <dd className="mt-1 text-sm text-gray-900">
                        {project.mainSystems && project.mainSystems.length > 0 ? (
                          <div className="flex flex-wrap gap-2">
                            {project.mainSystems.map((system: any) => (
                              <span
                                key={system.id}
                    ...
                    <div>
                      ...
                      <dd className="mt-1 text-sm text-gray-900">
                        {project.subSystems && project.subSystems.length > 0 ? (
                          <div className="flex flex-wrap gap-2">
                            {project.subSystems.map((subSystem: any) => (
                              <span
                                key={subSystem.id}
            ...
            {activeTab === 'files' && (
              <FilesTab projectId={projectId} projectNumber={project.projectNumber} filesCount={project.filesCount} />
            )}
          </div>
        </div>
        </>
    ...
    <AddDrawingDrawer
      ...
      projectId={projectId}
      projectSubSystems={project?.subSystems}
    />
      ...
      subSystems={
        project?.subSystems && project.subSystems.length > 0
          ? (subSystemsData || []).filter(subSystem =>
              project.subSystems?.some(projectSubSystem => projectSubSystem.id === subSystem.id)
            )
          : subSystemsData || []
      }
