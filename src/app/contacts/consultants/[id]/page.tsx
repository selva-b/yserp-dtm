...
import { useConsultant, useDeleteConsultant, useToggleConsultantVerified } from '@/hooks/use-consultants'
 ...
 *
 * Features:
 * - View complete consultant organization details
...
export default function ConsultantDetailsPage() {
  ...
  // Data fetching
  const { data: consultant, isLoading, error } = useConsultant(id)

  // Mutations
  const { mutate: deleteConsultant, isPending: isDeleting } = useDeleteConsultant()
  const { mutate: toggleVerified } = useToggleConsultantVerified()

  // Handlers
  const handleBack = () => {
    router.push('/contacts/consultants')
  }
    ...
    return (
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-sm text-gray-600">Loading consultant details...</p>
        </div>
      </div>
      </AppLayout>
    )
        ...
        <div className="px-6 py-8">
          <div className="bg-white shadow sm:rounded-lg p-6">
            <div className="text-center">
              ...
              <h3 className="mt-2 text-sm font-medium text-gray-900">Consultant Not Found</h3>
              <p className="mt-1 text-sm text-gray-500">
                The consultant you're looking for does not exist or has been deleted.
              </p>
              <div className="mt-6">
                <button
                  onClick={handleBack}
  ...
  const primaryContact = consultant.contactPersons?.find((cp: any) => cp.isPrimary)
  const additionalContacts = consultant.contactPersons?.filter((cp: any) => !cp.isPrimary) || []

  // Check if we have business registration info
  const hasBusinessInfo = consultant.registrationNumber || consultant.taxId ||
    consultant.licenseNumber || consultant.gstNumber || consultant.panNumber
  ...
  return (
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <div>
                <h1 className="text-2xl font-bold text-gray-900">{consultant.companyName}</h1>
          ...
          <div className="lg:col-span-2 space-y-6">
              ...
              <div className="px-4 py-5 sm:p-6">
                ...
                <dl className="grid grid-cols-1 gap-x-4 gap-y-4 sm:grid-cols-2">
                  <div>
                    ...
                    <dd className="mt-1 text-sm text-gray-900">
                      {consultant.email ? (
                        <a href={`mailto:${consultant.email}`} className="text-blue-600 hover:text-blue-700">
                          {consultant.email}
                        </a>
                      ) : (
                        '-'
                  ...
                  <div>
                    ...
                    <dd className="mt-1 text-sm text-gray-900">
                      {consultant.primaryPhone ? (
                        <a href={`tel:${consultant.primaryPhone}`} className="text-blue-600 hover:text-blue-700">
                          {consultant.primaryPhone}
                        </a>
                      ) : (
                        '-'
                  ...
                  <div>
                    ...
                    <dd className="mt-1 text-sm text-gray-900">{consultant.secondaryPhone || '-'}</dd>
                  </div>
                  <div>
              ...
              <div className="px-4 py-5 sm:p-6">
                ...
                <dl className="grid grid-cols-1 gap-x-4 gap-y-4 sm:grid-cols-2">
                  <div className="sm:col-span-2">
                    ...
                    <dd className="mt-1 text-sm text-gray-900">{consultant.address || '-'}</dd>
                  </div>
                  <div>
                    <dt className="text-sm font-medium text-gray-500">City</dt>
                    <dd className="mt-1 text-sm text-gray-900">{consultant.city || '-'}</dd>
                  </div>
                  <div>
                    <dt className="text-sm font-medium text-gray-500">State / Province</dt>
                    <dd className="mt-1 text-sm text-gray-900">{consultant.state || '-'}</dd>
                  </div>
                  <div>
                    <dt className="text-sm font-medium text-gray-500">Country</dt>
                    <dd className="mt-1 text-sm text-gray-900">{consultant.country || '-'}</dd>
                  </div>
                  <div>
                    <dt className="text-sm font-medium text-gray-500">Postal Code</dt>
                    <dd className="mt-1 text-sm text-gray-900">{consultant.postalCode || '-'}</dd>
                  </div>
                </dl>
              </div>
            </div>

            {/* Business Registration (Consultants only) */}
            {hasBusinessInfo && (
              <div className="bg-white shadow sm:rounded-lg">
                <div className="px-4 py-5 sm:p-6">
                  <h3 className="text-lg leading-6 font-medium text-gray-900 mb-4">
                  ...
                  <dl className="grid grid-cols-1 gap-x-4 gap-y-4 sm:grid-cols-2">
                    {consultant.registrationNumber && (
                      <div>
                        ...
                        <dd className="mt-1 text-sm text-gray-900">{consultant.registrationNumber}</dd>
                      </div>
                    )}
                    {consultant.taxId && (
                      <div>
                        <dt className="text-sm font-medium text-gray-500">Tax ID</dt>
                        <dd className="mt-1 text-sm text-gray-900">{consultant.taxId}</dd>
                      </div>
                    )}
                    {consultant.licenseNumber && (
                      <div>
                        <dt className="text-sm font-medium text-gray-500">License Number</dt>
                        <dd className="mt-1 text-sm text-gray-900">{consultant.licenseNumber}</dd>
                      </div>
                    )}
                    {consultant.gstNumber && (
                      <div>
                        <dt className="text-sm font-medium text-gray-500">GST Number</dt>
                        <dd className="mt-1 text-sm text-gray-900">{consultant.gstNumber}</dd>
                      </div>
                    )}
                    {consultant.panNumber && (
                      <div>
                        <dt className="text-sm font-medium text-gray-500">PAN Number</dt>
                        <dd className="mt-1 text-sm text-gray-900">{consultant.panNumber}</dd>
                      </div>
                    )}
                  </dl>
                </div>
            ...
            {additionalContacts.length > 0 ? (
              <div className="bg-white shadow sm:rounded-lg">
                <div className="px-4 py-5 sm:p-6">
                  <h3 className="text-lg leading-6 font-medium text-gray-900 mb-4">
                    Contact Persons ({consultant.contactPersons?.length || 0})
                  </h3>
              ...
              <div className="px-4 py-5 sm:p-6">
                ...
                <dl className="space-y-4">
                  ...
                  <div>
                    ...
                    <dd className="mt-1 text-sm text-gray-900">
                      {consultant.contactPersons?.length || 0} contact
                      {consultant.contactPersons?.length !== 1 ? 's' : ''}
                    </dd>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <dd className="mt-1 text-sm text-gray-900">
                      {consultant.updatedAt
                        ? new Date(consultant.updatedAt).toLocaleDateString('en-US', {
                            year: 'numeric',
                            month: 'long',
                            day: 'numeric',
                          })
      ...
      <ConfirmDialog
        ...
        message={`Are you sure you want to delete "${consultant.companyName}"? This action cannot be undone.`}
        confirmText="Delete"
        cancelText="Cancel"
        variant="danger"
