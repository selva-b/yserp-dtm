...
import { useProjectsReport, useExportReport, useUsersCatalog, useMainSystemsCatalog } from '@/hooks/use-reports'
...
import type { ProjectsFilterParams, ExportFormat } from '@/types/reports'
...
/**
 * Enhanced Projects Report Page (Phase 5)
 */
export default function ProjectsReportPage() {
  const [filters, setFilters] = useState<ProjectsFilterParams>({
    limit: 25,
    sortBy: 'created_at',
    sortDir: 'desc',
    startDateFrom: getCurrentMonthStart(),
    startDateTo: getCurrentDate(),
  })
  const [showCharts, setShowCharts] = useState(false)
  const [showFilterDialog, setShowFilterDialog] = useState(false)
  const [showMetrics, setShowMetrics] = useState(false)

  const { data, isLoading, error } = useProjectsReport(filters)
  ...
  const metrics = useMemo(() => {
    ...
    const totalDrawings = data.data.reduce((sum, p) => sum + p.drawings_count, 0)
    const totalTickets = data.data.reduce((sum, p) => sum + p.tickets_count, 0)
    const avgDrawings = total > 0 ? Math.round(totalDrawings / total) : 0

    // Task counts
    const totalTasks = data.data.reduce((sum, p) => sum + p.tasks_count, 0)
    const totalPendingTasks = data.data.reduce((sum, p) => sum + p.tasks_pending_count, 0)
    const totalInProgressTasks = data.data.reduce((sum, p) => sum + p.tasks_in_progress_count, 0)
    const totalCompletedTasks = data.data.reduce((sum, p) => sum + p.tasks_completed_count, 0)
    const totalCancelledTasks = data.data.reduce((sum, p) => sum + p.tasks_cancelled_count, 0)
  ...
  const filterFields: ReportFilterField[] = [
    ...
    },
    {
      ...
      value: filters.projectManagerIds,
    },
    {
  ...
  const activeFilterCount = useMemo(() => {
    ...
    if (filters.projectManagerIds && filters.projectManagerIds.length > 0) count++
  ...
  const handleApplyFilters = (newFilters: Record<string, any>) => {
    setFilters(prev => ({
      ...
      projectManagerIds: newFilters.projectManagerIds,
  ...
  const handleExport = async (format: ExportFormat) => {
    try {
      await exportMutation.mutateAsync({ reportType: 'projects', format, filters })
    } catch (err) {
  ...
  const columns = useMemo<ColumnDef<any>[]>(() => [
    {
      id: 'display_number',
      header: 'Project #',
      accessor: (row) => row.display_number || row.project_number,
      sortable: true,
      sortField: 'display_number',
      cell: (row) => (
        <Link href={`/projects/${row.id}/view`} className="text-blue-600 hover:text-blue-800 hover:underline font-medium">
          {row.display_number || row.project_number}
        </Link>
      ),
    },
    {
    ...
    },
    {
      id: 'managers',
      header: 'Managers',
      accessor: (row) => row.project_manager_name || row.drafting_manager_name,
      hideOnMobile: true,
      cell: (row) => (
        <div className="flex flex-col text-xs">
          {row.project_manager_name && <span>PM: {row.project_manager_name}</span>}
    ...
    },
    {
      ...
      sortable: true,
      cell: (row) => row.project_start_date ? new Date(row.project_start_date).toLocaleDateString() : '—',
    },
    {
      id: 'project_closing_date',
      header: 'Closing Date',
      accessor: 'project_closing_date',
      sortable: true,
      cell: (row) => row.project_closing_date ? new Date(row.project_closing_date).toLocaleDateString() : '—',
    },
    {
      id: 'drawings_count',
      header: 'Drawings',
    ...
    {
      id: 'tasks_cancelled_count',
      header: 'Cancelled',
      accessor: 'tasks_cancelled_count',
      align: 'center',
      hideOnMobile: true,
      cell: (row) => (
        <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-800">
          {row.tasks_cancelled_count}/{row.tasks_count}
        </span>
      ),
    },
  ], [])
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-gray-900">Projects Report</h2>
        <p className="mt-1 text-sm text-gray-600">View projects data, metrics, and performance</p>
      </div>

      {metrics && (
        <MetricsGrid expanded={showMetrics}>
          <MetricsCard title="Total Projects" value={metrics.total} />
      ...
      {isLoading && (
        <div className="bg-white shadow rounded-lg p-12 text-center">
          ...
          <p className="text-gray-600">Loading projects report...</p>
        </div>
      )}
